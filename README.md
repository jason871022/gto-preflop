# ♠ GTO 翻前範圍工具（Preflop Ranges & Trainer）

一個**純前端、免安裝**的德州撲克 GTO 學習工具：翻前範圍查詢、隨機練習、Equity 計算，還有翻後決策與嵌入式真 solver。開瀏覽器就能用，桌機與手機皆可。

> ⚠️ 本工具是**學習用起點**。翻前為固定範圍表、翻後決策為「基本原則」建議，皆非針對對手的精確剝削策略；精確 GTO 翻後解請用內建的「翻後 Solver」。
...
| **隨機練習** | 依情境出題（加注／3-bet／4-bet／跟注／蓋牌），**鍵盤快捷**（1/2/3 + 空白鍵）、**限時 60 秒挑戰**、**錯題本複習**、計分連對 |
| **Equity** | 範圍 vs 範圍勝率（蒙地卡羅）、**逐手熱圖**、可**指定公牌**算翻後 equity |
| **翻後決策** | 選角色/位置/人數 + 手牌 + 公牌 → 牌面材質、成手/聽牌判斷、equity、啟發式下注/過牌建議（附「隨機情境練習」） |
| **翻後 Solver** | 嵌入開源 [WASM Postflop](https://github.com/b-inary/wasm-postflop)，真正用 Discounted CFR 即時求解 |

---

## 🕹️ 線上遊玩（GitHub Pages）

部署後（見下方步驟），任何裝置的瀏覽器打開這個網址即可，**手機也行**：

```
https://<你的帳號>.github.io/gto-preflop/
```
...
- **翻後 Solver**：[WASM Postflop / postflop-solver](https://github.com/b-inary/wasm-postflop) by b-inary（AGPL-3.0）— 本專案僅以 iframe 嵌入其線上版，未修改或散布其原始碼。
